...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['login'])) {
    ...
    $admin_email = test_input($_POST["username"]);
    $admin_password = test_input($_POST["password"]);
    $stmt = $conn->prepare('SELECT * FROM tbl_admin');
    $stmt->execute();
    $users = $stmt->fetch(PDO::FETCH_ASSOC);
          
        if(($users['admin_email'] == $admin_email) && ($users['admin_password'] == $admin_password) ) {
            $_SESSION['admin_login_email']=$users['admin_email'];
            $_SESSION['admin_login_name']=$users['admin_name'];
